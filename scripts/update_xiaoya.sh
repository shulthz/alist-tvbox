...
IMAGE_ID=$(docker images -q docker.1panel.live/haroldli/xiaoya-tvbox:${TAG})
echo -e "\e[32m下载最新Docker镜像，平台：${platform}, image tag: ${TAG}\e[0m"
for i in 1 2 3 4 5
do
   docker pull --platform ${platform} docker.1panel.live/haroldli/xiaoya-tvbox:${TAG} && break
done

NEW_IMAGE=$(docker images -q docker.1panel.live/haroldli/xiaoya-tvbox:${TAG})
